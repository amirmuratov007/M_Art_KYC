...
export function normalizeObject(payload = {}) {
  return {
    ...
    source_request_id: String(payload.source_request_id || '').trim()
  }
}
...
export async function findRiskObjectBySourceRequest(sourceRequestId) {
  ...
  if (!source) return { ok: false, mode: 'validation', object: null, error: 'Missing source_request_id' }
  ...
  const { data, error } = await supabase
    .from(TABLE_OBJECTS)
    .select('*')
    .eq('source_request_id', source)
    .order('updated_at', { ascending: false })
    .limit(1)
    .maybeSingle()

  if (error) return errorResponse(error, { object: null })
