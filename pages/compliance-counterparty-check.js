...
const signals = ["sanctions, PEP and adverse media signals", "beneficial owners, connected parties and corporate perimeter", "payment route and jurisdictional exposure", "indirect risk without a direct list match"]
const useCases = ["before an international deal or payment", "before onboarding a new client or partner", "when there is no direct sanctions match but context is risky"]
...
export default function Page() {
  ...
  return (
    <>
      <Head>
        <title>Compliance Counterparty Check | HEIMDALL</title>
        <meta name="description" content="Compliance counterparty check: sanctions, PEP, adverse media, ownership, payment route, reputation and regulatory exposure." />
        <link rel="canonical" href="https://www.heimdall-group.ru/compliance-counterparty-check" />
      </Head>
          ...
          <div className="grid gap-6 lg:grid-cols-[0.82fr_1fr]">
            ...
            <div className="rounded-[36px] border border-white/10 bg-white/[0.045] p-8 backdrop-blur-2xl">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/cases-en/sanctions-risk-without-direct-match" className="inline-flex items-center gap-2 rounded-2xl border border-[#D6A84F]/25 bg-[#D6A84F]/10 px-5 py-3 text-sm font-semibold text-[#F7D784]">
                  Read related case: Case: sanctions risk without direct match <ArrowRight className="h-4 w-4" />
                </Link>
      ...
      <ContactModal open={open} onClose={() => setOpen(false)} language="en" defaultTopic="Compliance Counterparty Check" />
    </>
  )
}
