...
const useCases = ["перед авансом китайскому поставщику", "перед первой поставкой или OEM-производством", "если цена слишком хорошая или документы вызывают сомнения"]
...
export default function Page() {
  ...
  return (
    <>
      <Head>
        <title>Проверка поставщика из Китая | HEIMDALL</title>
        <meta name="description" content="Проверка китайского поставщика перед оплатой: регистрация, производственный профиль, сайт, документы, реквизиты, посредники и риск фиктивности." />
        <link rel="canonical" href="https://www.heimdall-group.ru/proverka-postavshchika-iz-kitaya" />
      </Head>
        ...
        <section className="relative z-10 mx-auto grid max-w-7xl gap-12 px-4 py-20 sm:px-5 md:py-24 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            ...
            <h1 className="mt-9 text-5xl font-semibold leading-[0.95] tracking-[-0.06em] md:text-8xl">
              Проверка поставщика из Китая перед авансом
            </h1>

            <p className="mt-8 max-w-3xl text-lg leading-8 text-white/64 md:text-xl md:leading-9">
              Помогаем понять, реальный ли это производитель, торговый посредник или рискованная структура перед переводом денег.
            </p>
            ...
            <div className="rounded-[36px] border border-white/10 bg-white/[0.045] p-8 backdrop-blur-2xl">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/cases/china-supplier-advance-payment" className="inline-flex items-center gap-2 rounded-2xl border border-[#D6A84F]/25 bg-[#D6A84F]/10 px-5 py-3 text-sm font-semibold text-[#F7D784]">
                  Читать связанный кейс: Кейс: поставщик из Китая перед авансом <ArrowRight className="h-4 w-4" />
                </Link>
      ...
      <ContactModal open={open} onClose={() => setOpen(false)} language="ru" defaultTopic="Проверка поставщика из Китая" />
    </>
  )
}
