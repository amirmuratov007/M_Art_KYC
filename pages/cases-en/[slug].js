...
export async function getStaticPaths() {
  return {
    paths: caseStudiesEn.map((item) => ({ params: { slug: item.slug } })),
    fallback: false
  }
}
...
export default function CaseDetailPageEn({ study }) {
  return (
    <>
      <Head>
        <title>{study.title} | HEIMDALL</title>
        <meta name="description" content={study.summary} />
        <link rel="canonical" href={`https://www.heimdall-group.ru/cases-en/${study.slug}`} />
      </Head>
      ...
      <main className="min-h-screen overflow-hidden bg-[#050816] text-white">
        ...
        <article className="relative z-10 mx-auto max-w-5xl px-4 py-20 sm:px-5">
          <Link href="/cases-en" className="inline-flex items-center gap-2 text-sm font-semibold text-sky-200">
          ...
          <section className="mt-12 grid gap-6">
            <div className="rounded-[34px] border border-white/10 bg-white/[0.045] p-8 backdrop-blur-2xl">
              <div className="text-sm uppercase tracking-[0.22em] text-[#F7D784]">Situation</div>
              <p className="mt-5 text-lg leading-9 text-white/68">{study.situation}</p>
            </div>

            <div className="rounded-[34px] border border-white/10 bg-white/[0.045] p-8 backdrop-blur-2xl">
              <div className="text-sm uppercase tracking-[0.22em] text-[#F7D784]">Risk signals</div>
            ...
            <div className="rounded-[34px] border border-[#D6A84F]/20 bg-[#D6A84F]/[0.07] p-8 backdrop-blur-2xl">
              <div className="text-sm uppercase tracking-[0.22em] text-[#F7D784]">Result</div>
