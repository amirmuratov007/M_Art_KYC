...
const blocks = [["Проверка контрагентов", "Скрытые владельцы, суды, санкции, репутация."], ["Проверка кандидатов", "Биография, конфликты интересов, репутационные сигналы."], ["Санкционный скрининг", "Прямые и косвенные связи с ограниченными лицами."], ["Проверка поставщиков", "Платёжная дисциплина, судебная история, конфликтные сигналы."], ["Проверка руководителей", "Проверка топ-менеджеров и чувствительных должностей."], ["Иностранные компании", "Данные из разных юрисдикций и реестров."]]
    ...
    <main className="min-h-screen bg-[#050816] text-white">
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-5 py-24">
        <div className="max-w-5xl">
          ...
          <h1 className="mt-10 text-5xl font-semibold leading-[0.95] tracking-[-0.06em] md:text-8xl">SEO-кластер под услуги проверки</h1>
          <p className="mt-10 max-w-3xl text-xl leading-9 text-white/64">Структурированные направления HEIMDALL для поискового трафика, рекламы и конкретных клиентских запросов.</p>
        </div>
      </section>
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {blocks.map(([title, text]) => (
            <div key={title} className="rounded-[34px] border border-white/10 bg-white/[0.045] p-7 backdrop-blur-2xl transition hover:-translate-y-2 hover:border-sky-300/35">
              ...
              <div className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-sky-200">Подробнее <ArrowRight className="h-4 w-4" /></div>
            </div>
          ))}
        </div>
      </section>
