...
const signals = ["registration, business profile and signs of real activity", "website, documents, contacts, addresses and bank-detail consistency", "manufacturing profile and intermediary indicators", "adverse media, export signals and compliance exposure"]
const useCases = ["before advance payment to a China supplier", "before first shipment or OEM production", "when pricing or documents look too good to be safe"]
...
export default function Page() {
  ...
  return (
    <>
      <Head>
        <title>China Supplier Verification | HEIMDALL</title>
        <meta name="description" content="China supplier verification before payment: registration, manufacturing profile, website, documents, bank details, intermediaries and shell risk." />
        <link rel="canonical" href="https://www.heimdall-group.ru/china-supplier-verification" />
      </Head>
        ...
        <section className="relative z-10 mx-auto grid max-w-7xl gap-12 px-4 py-20 sm:px-5 md:py-24 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            ...
            <h1 className="mt-9 text-5xl font-semibold leading-[0.95] tracking-[-0.06em] md:text-8xl">
              China supplier verification before advance payment
            </h1>

            <p className="mt-8 max-w-3xl text-lg leading-8 text-white/64 md:text-xl md:leading-9">
              We help determine whether you are dealing with a real manufacturer, trading intermediary or high-risk structure before money is sent.
            </p>
          ...
          <div className="grid gap-6 lg:grid-cols-[0.82fr_1fr]">
            ...
            <div className="rounded-[36px] border border-white/10 bg-white/[0.045] p-8 backdrop-blur-2xl">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/cases-en/china-supplier-advance-payment" className="inline-flex items-center gap-2 rounded-2xl border border-[#D6A84F]/25 bg-[#D6A84F]/10 px-5 py-3 text-sm font-semibold text-[#F7D784]">
                  Read related case: Case: China supplier before advance payment <ArrowRight className="h-4 w-4" />
                </Link>
      ...
      <ContactModal open={open} onClose={() => setOpen(false)} language="en" defaultTopic="China Supplier Verification" />
    </>
  )
}
