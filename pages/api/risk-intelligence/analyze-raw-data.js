...
- Если сырье содержит маркеры HTML TAB, анализируй каждую вкладку как отдельный источник/раздел и отражай важные вкладки в sourceContour или sourceRefs.
- Если встречаются маркеры IMAGE, отмечай наличие фото/изображений как материал для ручной проверки, но не делай выводов о лице или совпадении без OCR/face-review.
...
function buildSourceManifest(sources = []) {
  const list = normalizeList(sources, 20).map((source, index) => ({
    ...
    html_tabs: Array.isArray(source?.tabs) ? source.tabs.length : 0,
    tables: Number(source?.table_count || 0),
    details: Number(source?.details_count || 0),
    images: Number(source?.image_count || 0),
...
Если в тексте есть блоки "HEIMDALL SOURCE", используй их как паспорт источников. Если внутри есть "HTML TAB", считай каждую вкладку отдельным разделом исходного файла. В sourceRefs указывай имя файла, название вкладки или короткий фрагмент, но не выдумывай источники. Если источник имеет status needs_ocr, отметь его в reviewChecklist, но не делай выводов из невидимого текста.
