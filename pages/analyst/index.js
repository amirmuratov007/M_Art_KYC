...
export default function AnalystDashboard() {
  const active = analystCases.filter((item) => item.status !== 'Исполнено').length
  ...
  return (
    <AnalystLayout title="Панель аналитика">
      <div className="mb-8">
        <div className="text-sm uppercase tracking-[0.25em] text-sky-300/80">Консоль аналитика HEIMDALL</div>
        <h1 className="mt-4 text-5xl font-semibold tracking-[-0.06em] md:text-7xl">Панель аналитика</h1>
        <p className="mt-6 max-w-3xl text-lg leading-8 text-white/60">Внутреннее рабочее пространство для проверок, документов, заметок и подготовки черновиков отчетов.</p>
      </div>

      <div className="mb-8 grid gap-5 md:grid-cols-3">
        {[
          ['Активные проверки', active],
          ['Проверки высокого риска', highRisk],
          ['Средняя оценка риска', `${avgRisk}/100`]
        ].map(([label, value]) => (
