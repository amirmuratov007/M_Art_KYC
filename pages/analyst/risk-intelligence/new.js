  ...
  return (
    <AnalystLayout title="Создать объект риска">
      <div className="mb-8">
        ...
        <p className="mt-6 max-w-3xl text-lg leading-8 text-white/60">На первом этапе форма работает как безопасный каркас без записи в базу. После подключения базы эти поля станут основой таблицы объектов риска.</p>
      </div>
      ...
      <div className="rounded-[34px] border border-white/10 bg-white/[0.045] p-6 md:p-8">
        ...
        <div className="mt-8 rounded-2xl border border-[#D6A84F]/20 bg-[#D6A84F]/10 p-5 text-sm leading-7 text-[#F7D784]">
          Предпросмотр локального номера: {previewId}. В этом этапе сохранение в базу намеренно не включено, чтобы не сломать действующую CRM.
        </div>
        <button type="button" onClick={() => alert('Первый этап: объект не записывается в базу. Подключение базы будет отдельным безопасным этапом.')} className="mt-6 rounded-2xl bg-sky-500 px-7 py-4 font-semibold text-white">Создать демонстрационный объект</button>
      </div>
    </AnalystLayout>
  )
}
