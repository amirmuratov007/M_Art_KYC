        ...
        <section id="service-pages" className="relative z-10 mx-auto max-w-7xl px-5 pb-24">
          <div className="rounded-[42px] border border-white/10 bg-white/[0.045] p-8 backdrop-blur-2xl md:p-12">
            <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
              <div>
                <div className="text-sm uppercase tracking-[0.25em] text-sky-300/80">
                  {language === 'ru' ? 'Посадочные страницы' : 'Service landing pages'}
                </div>
                <h2 className="mt-4 text-4xl font-semibold tracking-[-0.04em] md:text-6xl">
                  {language === 'ru'
                    ? 'Глубокие страницы под каждую услугу'
                    : 'Dedicated pages for each intelligence service'}
                </h2>
                <p className="mt-6 text-lg leading-8 text-white/64">
                  {language === 'ru'
                    ? 'Они помогут запускать рекламу и собирать органический трафик по конкретным запросам клиентов.'
                    : 'These pages support paid campaigns and organic traffic for specific client search intents.'}
                </p>
              </div>
              ...
              <div className="grid gap-3">
                {(language === 'ru'
                  ? [
                      ...
                      ['/candidate-screening', 'Проверка кандидатов'],
                      ['/aml-kyc', 'AML/KYC проверки'],
                      ['/due-diligence', 'Комплексная проверка бизнеса'],
                      ['/foreign-company-check', 'Проверка иностранных компаний']
                    ]
                  : [
                      ['/counterparty-check', 'Counterparty Checks'],
                      ['/candidate-screening', 'Candidate Screening'],
                      ['/aml-kyc', 'AML/KYC Reviews'],
                      ['/due-diligence', 'Business Due Diligence'],
                      ['/foreign-company-check', 'Foreign Company Review']
                    ]
                ).map(([href, label]) => (
