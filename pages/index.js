  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-[#050816] text-white overflow-hidden">
        ...
        <section id="service-pages" className="relative z-10 mx-auto max-w-7xl px-5 pb-24">
          <div className="mb-12 max-w-4xl">
            <div className="text-sm uppercase tracking-[0.25em] text-sky-300/80">
              {language === 'ru' ? 'Направления HEIMDALL' : 'HEIMDALL Services'}
            </div>
            <h2 className="mt-4 text-4xl font-semibold tracking-[-0.04em] md:text-6xl">
              {language === 'ru'
                ? 'Проверки для принятия правильных решений'
                : 'Intelligence reviews for better decisions'}
            </h2>
            <p className="mt-6 text-lg leading-8 text-white/64">
              {language === 'ru'
                ? 'Каждое направление адаптируется под задачу, юрисдикцию, уровень риска и чувствительность кейса.'
                : 'Each engagement is adapted to the case, jurisdiction, risk profile and sensitivity level.'}
            </p>
          </div>
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {(language === 'ru'
              ? [
                  ['/counterparty-check', 'Проверка контрагентов', 'Проверка владельцев, судебных рисков, санкций и репутационного фона.'],
                  ['/candidate-screening', 'Проверка кандидатов', 'Screening кандидатов до передачи клиенту или оффера.'],
                  ['/aml-kyc', 'AML/KYC', 'Проверка клиентов, партнёров и бенефициаров на комплаенс-риски.'],
                  ['/due-diligence', 'Due Diligence', 'Комплексный анализ бизнеса перед сделкой или инвестицией.'],
                  ['/foreign-company-check', 'Иностранные компании', 'Проверка зарубежных компаний и связанных структур.'],
                  ['/#lead', 'Индивидуальные кейсы', 'Сложные проверки, чувствительные ситуации и нестандартные запросы.']
                ]
              : [
                  ['/counterparty-check', 'Counterparty Checks', 'Ownership, litigation, sanctions and reputation review.'],
                  ['/candidate-screening', 'Candidate Screening', 'Candidate screening before client submission or offer stage.'],
                  ['/aml-kyc', 'AML/KYC', 'Compliance-focused reviews for clients, partners and beneficial owners.'],
                  ['/due-diligence', 'Due Diligence', 'Business intelligence before transactions or investments.'],
                  ['/foreign-company-check', 'Foreign Company Reviews', 'Cross-border checks for overseas companies and structures.'],
                  ['/#lead', 'Custom Intelligence Cases', 'Sensitive, complex and non-standard review requests.']
                ]
            ).map(([href, title, text]) => (
              <Link
                key={href}
                href={href}
                className="group overflow-hidden rounded-[34px] border border-white/10 bg-white/[0.045] p-7 backdrop-blur-2xl transition duration-500 hover:-translate-y-2 hover:border-sky-300/35 hover:bg-white/[0.07]"
              >
                <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-2xl border border-sky-300/20 bg-sky-300/10 text-sky-200">
                  <ShieldCheck className="h-5 w-5" />
                </div>

                <h3 className="text-2xl font-semibold text-white transition group-hover:text-sky-100">
                  {title}
                </h3>
