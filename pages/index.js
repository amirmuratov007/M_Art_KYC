...
const content = {
  ru: {
    ...
    form: {
      ...
      duplicate: 'Заявка уже отправляется. Подождите несколько секунд.'
    },
  ...
  en: {
    ...
    form: {
      ...
      duplicate: 'Request is already being sent. Please wait a few seconds.'
    },
...
export default function Home() {
  ...
  const submitLead = async (event) => {
    ...
    if (status === 'loading') {
      console.warn('[HEIMDALL_FORM] duplicate submit blocked')
      setMessage(t.form.duplicate)
      return
    }

    setMessage('')
    console.log('[HEIMDALL_FORM] submitLead started', { locale: language, check_type: form.check_type })

    if (!form.name.trim() || !form.phone.trim()) {
      console.warn('[HEIMDALL_FORM] validation failed: missing name or phone')
      setStatus('error')
    ...
    try {
      ...
      console.log('[HEIMDALL_FORM] /api/lead response status', response.status)

      const data = await response.json().catch((jsonError) => {
        console.error('[HEIMDALL_FORM] response JSON parse failed', jsonError)
        return {}
      })
      console.log('[HEIMDALL_FORM] /api/lead response body', data)

      if (!response.ok || !data.ok) {
        throw new Error(data.error || 'Send error')
      }

      console.log('[HEIMDALL_FORM] lead sent successfully', { lead_id: data.lead_id, telegram_ok: data.telegram?.ok })
      setStatus('success')
      ...
      setStatus('error')
      setMessage(error.name === 'AbortError' ? `${t.form.error} Timeout.` : t.form.error)
    }
  }
      ...
      <main className="min-h-screen bg-[#050816] text-white overflow-hidden">
        ...
        <section id="lead" className="relative z-10 mx-auto max-w-7xl px-5 pb-24">
          <div className="grid gap-12 rounded-[42px] border border-sky-300/20 bg-gradient-to-br from-sky-500/12 via-white/[0.04] to-transparent p-6 backdrop-blur-2xl md:p-12 lg:grid-cols-[0.9fr_1.1fr]">
            ...
            <form onSubmit={submitLead} noValidate className="grid gap-4">
              <div className="grid gap-4 md:grid-cols-2">
              ...
              <button type="button" onClick={submitLead} disabled={status === 'loading'} className="rounded-2xl bg-sky-500 px-7 py-4 font-semibold text-white shadow-[0_0_45px_rgba(56,189,248,0.30)] transition hover:bg-sky-400 disabled:cursor-not-allowed disabled:opacity-60">
