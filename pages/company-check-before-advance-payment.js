...
const signals = ["shell-company or intermediary indicators", "links to distressed or risky legal entities", "disputes, debt and non-performance history", "document, bank detail and communication inconsistencies"]
const useCases = ["before prepayment to a new supplier", "before international supply payment", "when the counterparty pressures for urgent transfer"]
...
export default function Page() {
  ...
  return (
    <>
      <Head>
        <title>Company Check Before Advance Payment | HEIMDALL</title>
        <meta name="description" content="Company check before advance payment: shell indicators, litigation risk, ownership, links, payment risk and documents." />
        <link rel="canonical" href="https://www.heimdall-group.ru/company-check-before-advance-payment" />
      </Head>
        ...
        <section className="relative z-10 mx-auto grid max-w-7xl gap-12 px-4 py-20 sm:px-5 md:py-24 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            ...
            <h1 className="mt-9 text-5xl font-semibold leading-[0.95] tracking-[-0.06em] md:text-8xl">
              Company check before advance payment
            </h1>

            <p className="mt-8 max-w-3xl text-lg leading-8 text-white/64 md:text-xl md:leading-9">
              Advance payment is often the point of no return. We review the company before money is sent.
            </p>
            ...
            <div className="rounded-[36px] border border-white/10 bg-white/[0.045] p-8 backdrop-blur-2xl">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/cases-en/china-supplier-advance-payment" className="inline-flex items-center gap-2 rounded-2xl border border-[#D6A84F]/25 bg-[#D6A84F]/10 px-5 py-3 text-sm font-semibold text-[#F7D784]">
                  Read related case: Case: stopped advance payment <ArrowRight className="h-4 w-4" />
                </Link>
      ...
      <ContactModal open={open} onClose={() => setOpen(false)} language="en" defaultTopic="Company Check Before Advance Payment" />
    </>
  )
}
