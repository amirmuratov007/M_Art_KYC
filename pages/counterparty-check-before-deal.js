...
const signals = ["ownership structure, beneficial owners and connected parties", "litigation background, enforcement records and debt indicators", "sanctions and reputation exposure", "red flags that may change deal terms"]
const useCases = ["before a major contract or partnership", "before deferred payment or access transfer", "before stake, asset or business acquisition"]
...
export default function Page() {
  ...
  return (
    <>
      <Head>
        <title>Counterparty Check Before Deal | HEIMDALL</title>
        <meta name="description" content="Counterparty check before a deal: owners, litigation, sanctions, links, reputation, financial and corporate risk signals." />
        <link rel="canonical" href="https://www.heimdall-group.ru/counterparty-check-before-deal" />
      </Head>
        ...
        <section className="relative z-10 mx-auto grid max-w-7xl gap-12 px-4 py-20 sm:px-5 md:py-24 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            ...
            <h1 className="mt-9 text-5xl font-semibold leading-[0.95] tracking-[-0.06em] md:text-8xl">
              Counterparty check before a deal
            </h1>

            <p className="mt-8 max-w-3xl text-lg leading-8 text-white/64 md:text-xl md:leading-9">
              Before signing, it is important to understand not only the legal entity, but also the people, links and history around it.
            </p>
          ...
          <div className="grid gap-6 lg:grid-cols-[0.82fr_1fr]">
            ...
            <div className="rounded-[36px] border border-white/10 bg-white/[0.045] p-8 backdrop-blur-2xl">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/cases-en/counterparty-180m-contract" className="inline-flex items-center gap-2 rounded-2xl border border-[#D6A84F]/25 bg-[#D6A84F]/10 px-5 py-3 text-sm font-semibold text-[#F7D784]">
                  Read related case: Case: major equipment contract <ArrowRight className="h-4 w-4" />
                </Link>
      ...
      <ContactModal open={open} onClose={() => setOpen(false)} language="en" defaultTopic="Counterparty Check Before Deal" />
    </>
  )
}
