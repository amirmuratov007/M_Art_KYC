...
const categories = ['All', 'Контрагенты', 'Кандидаты', 'Бенефициары', 'Санкции', 'Рекрутинг', 'Due Diligence']
...
export default function JournalPage() {
  ...
  const filteredPosts = useMemo(() => {
    ...
    return telegramPosts.filter((post) => {
      ...
      const byQuery = !q || post.title.toLowerCase().includes(q) || post.text.toLowerCase().includes(q) || post.category.toLowerCase().includes(q)
  ...
  return (
    <>
      <Head>
        <title>Telegram Publications | HEIMDALL</title>
        ...
        <header className="relative z-10 border-b border-white/10 bg-[#050816]/70 backdrop-blur-2xl">
          <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5">
            ...
            <div className="flex items-center gap-3">
              <Link href="/telegram" className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-white/80">Telegram</Link>
        ...
        <section className="relative z-10 mx-auto max-w-7xl px-5 py-20 md:py-24">
          <div className="max-w-5xl">
            ...
            <h1 className="mt-10 text-5xl font-semibold leading-[0.95] tracking-[-0.06em] md:text-8xl">
              Telegram Channel Publications
            </h1>
            <p className="mt-10 max-w-3xl text-xl leading-9 text-white/64">
              HEIMDALL publications on counterparty intelligence, candidate screening, beneficial ownership, sanctions risk, due diligence and corporate security.
            </p>
          </div>
        </section>
        ...
        <section className="relative z-10 mx-auto max-w-7xl px-5 pb-10">
          <div className="grid gap-5 rounded-[34px] border border-white/10 bg-white/[0.045] p-5 backdrop-blur-2xl md:grid-cols-[1fr_auto] md:items-center">
            <div className="relative">
              ...
              <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search publications" className="w-full rounded-2xl border border-white/10 bg-black/25 px-12 py-4 text-white outline-none placeholder:text-white/35 focus:border-sky-300/40" />
            </div>
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {filteredPosts.map((post) => (
              <article key={post.slug} className="group rounded-[34px] border border-white/10 bg-white/[0.045] p-7 backdrop-blur-2xl transition duration-500 hover:-translate-y-2 hover:border-sky-300/35 hover:bg-white/[0.07]">
                ...
                <Link href={`/journal-en/${post.slug}`} className="mt-8 inline-flex items-center gap-2 text-sm font-semibold text-sky-200">
                  Read publication
        ...
        <section className="relative z-10 mx-auto max-w-7xl px-5 pb-32">
          <div className="grid gap-8 rounded-[42px] border border-sky-300/20 bg-gradient-to-br from-sky-500/12 via-white/[0.04] to-transparent p-10 backdrop-blur-2xl md:p-16 lg:grid-cols-[1fr_auto] lg:items-center">
            <div>
              ...
              <h2 className="mt-5 max-w-4xl text-4xl font-semibold tracking-[-0.05em] md:text-6xl">Follow the channel</h2>
              <p className="mt-6 max-w-3xl text-lg leading-8 text-white/64">
                Read short notes about counterparty checks, candidate screening, beneficial ownership and corporate risks.
              </p>
            </div>
            <a href="https://t.me/heimdall_risk" className="inline-flex items-center justify-center gap-3 rounded-2xl bg-sky-500 px-7 py-4 font-semibold text-white shadow-[0_0_45px_rgba(56,189,248,0.30)]">
              Open channel
