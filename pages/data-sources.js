...
const controls = [
  'Источники используются по категориям риска, а не механически одинаково для каждого запроса.',
  'Слабые совпадения маркируются как требующие проверки, а не как установленный факт.',
  'Отчет показывает существенные риски и контекст, а не раскрывает внутреннюю карту поиска.',
]
  ...
  return (
    <>
      <Head>
        ...
        <link rel="canonical" href="https://www.heimdall-group.ru/data-sources" />
      </Head>
        ...
        <section className="relative z-10 mx-auto grid max-w-7xl gap-6 px-4 pb-28 sm:px-5 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="rounded-[36px] border border-white/10 bg-white/[0.045] p-8 backdrop-blur-2xl">
            ...
            <div className="grid gap-4">
              {controls.map((item) => <p key={item} className="rounded-2xl border border-white/10 bg-black/20 p-4 text-sm leading-7 text-white/62">{item}</p>)}
            </div>
            <Link href="/privacy" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-[#F7D784]">Конфиденциальность <ArrowRight className="h-4 w-4" /></Link>
          </div>
        </section>
