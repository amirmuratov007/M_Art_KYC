...
export default function RiskTestPage() {
  ...
  return (
    <>
      <Head>
        <title>Бесплатные риск-тесты HEIMDALL | Сделки, поставщики, ИБ и внутренняя безопасность</title>
        <meta name="description" content="Выберите риск-тест HEIMDALL: сделка перед оплатой, поставщик, информационная безопасность, внутренние нарушения или внешняя служба безопасности. Получите предварительный риск-профиль." />
      ...
      <main className="min-h-screen overflow-hidden bg-[#050816] text-white">
        ...
        <HeimdallNav language="ru" languageHref={selected ? `/risk-test-en?test=${encodeURIComponent(selected)}` : '/risk-test-en'} />
        {exists ? <HeimdallRiskTest language="ru" testId={selected} /> : <HeimdallRiskTestHub language="ru" />}
