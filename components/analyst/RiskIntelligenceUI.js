...
export function RiskObjectCard({ item, onOpen }) {
  const open = () => {
    ...
    if (typeof window !== 'undefined') {
      window.location.href = `/analyst/risk-intelligence?open=${encodeURIComponent(item.id)}`
    }
  }

  return (
    <button type="button" onClick={open} className="group block w-full rounded-[32px] border border-white/10 bg-white/[0.045] p-6 text-left backdrop-blur-2xl transition hover:-translate-y-1 hover:border-sky-300/30">
