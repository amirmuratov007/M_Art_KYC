...
export default function ContactModal({ open, onClose, language = 'ru' }) {
  ...
  const [form, setForm] = useState({
    ...
    company: '',
    contact: '',
    topic: ru ? 'Общий запрос' : 'General request',
    message: ''
  })
      ...
      <div className="relative max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[34px] border border-white/10 bg-[#07101f] p-6 shadow-2xl md:p-8">
        ...
        <div className="pr-12">
          ...
          <p className="mt-4 text-sm leading-7 text-white/60">
            {ru ? 'Заполните форму. Заявка сразу придёт в Telegram-бот.' : 'Fill in the form. The request will go directly to the Telegram bot.'}
          </p>
        </div>
        ...
        {status === 'error' && (
          <div className="mt-5 rounded-2xl border border-red-300/20 bg-red-300/10 p-4 text-sm leading-6 text-red-100">
            {ru ? 'Не удалось отправить заявку. Проверьте переменные Telegram в Vercel.' : 'Could not send request. Check Telegram variables in Vercel.'}
          </div>
        )}
      </div>
    </div>
