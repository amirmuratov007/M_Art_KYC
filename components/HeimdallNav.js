...
const ruMenu = [
  ['Услуги', '/services', [['Проверка контрагентов', '/proverka-kontragenta'], ['Проверка кандидатов', '/proverka-kandidatov'], ['Проверка бенефициаров', '/proverka-beneficiarov'], ['AML / KYC', '/aml-kyc-russia'], ['Due Diligence', '/due-diligence-russia']]],
  ['Сопровождение', '/business-support', [['Комплексное сопровождение', '/business-support'], ['Клиентское приложение', '/client-app'], ['Прайс-лист сопровождения', '/business-support#pricing']]],
  ['Материалы', '/journal', [['Публикации Telegram', '/journal'], ['Примеры отчётов', '/sample-reports'], ['Кейсы', '/cases']]]
]

const enMenu = [
  ['Services', '/services-en', [['Counterparty Intelligence', '/corporate-intelligence'], ['Candidate Screening', '/background-check'], ['Beneficial Ownership', '/proverka-beneficiarov'], ['AML / KYC', '/aml-kyc-russia'], ['Due Diligence', '/due-diligence-dubai']]],
  ['Support', '/business-support-en', [['Business Support', '/business-support-en'], ['Client Application', '/client-app-en'], ['Support Pricing', '/business-support-en#pricing']]],
  ['Resources', '/journal-en', [['Telegram Publications', '/journal-en'], ['Sample Reports', '/sample-reports-en'], ['Cases', '/cases-en']]]
]
...
export default function HeimdallNav({ language }) {
  ...
  const detected = router.pathname === '/en' || router.pathname.endsWith('-en') ? 'en' : 'ru'
          ...
          <nav className="hidden items-center gap-1 lg:flex">
            {menu.map(([title, href, items]) => (
              <div key={title} className="group relative">
                <Link href={href} className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm text-white/70 transition hover:bg-white/7 hover:text-[#F7D784]">
                  {title}
                  <ChevronDown className="h-4 w-4 opacity-70" />
                </Link>

                <div className="pointer-events-none absolute left-1/2 top-full z-[9999] w-80 -translate-x-1/2 translate-y-4 rounded-[26px] border border-white/15 bg-[#050816] p-3 opacity-0 shadow-[0_35px_100px_rgba(0,0,0,0.88)] ring-1 ring-sky-300/10 transition duration-200 group-hover:pointer-events-auto group-hover:translate-y-3 group-hover:opacity-100">
