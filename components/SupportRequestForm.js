    ...
    <form onSubmit={submit} className="rounded-[42px] border border-sky-300/20 bg-white/[0.055] p-6 backdrop-blur-2xl md:p-8">
      <div className="mb-7">
        ...
        <p className="mt-4 text-sm leading-7 text-white/60">
          {ru ? 'Заполните короткую форму. Заявка сразу поступит в Telegram-бот HEIMDALL.' : 'Fill in a short form. The request will be sent directly to the HEIMDALL Telegram bot.'}
        </p>
      </div>
      ...
      <div className="grid gap-4">
        ...
        <input required value={form.company} onChange={(e) => update('company', e.target.value)} placeholder={ru ? 'Компания' : 'Company'} className="rounded-2xl border border-white/10 bg-black/25 px-5 py-4 text-white outline-none placeholder:text-white/35 focus:border-sky-300/40" />
      ...
      {status === 'error' && (
        <div className="mt-5 rounded-2xl border border-red-300/20 bg-red-300/10 p-4 text-sm leading-6 text-red-100">
          {ru ? 'Не удалось отправить заявку. Проверьте настройки Telegram-бота.' : 'Could not send request. Check Telegram bot settings.'}
        </div>
      )}
    </form>
  )
