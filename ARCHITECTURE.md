...
- `/api/telegram-miniapp-lead` validates signed Telegram `initData` with Telegram's Ed25519 public key and the Risk Radar bot ID. An optional dedicated token supports legacy clients; the notification bot token remains a separate integration. The API rate-limits requests and never stores the raw signed payload.
